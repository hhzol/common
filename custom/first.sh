...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/hhzol/common build/common
...
git clone -b main --depth 1 https://github.com/hhzol/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (hhzol/build-actions|hhzol/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/hhzol/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/hhzol/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="hhzol/build-actions"
  sudo rm -rf build common.sh
