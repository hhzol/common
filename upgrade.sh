...
function Diy_Part3() {
	...
	x86)
		if [[ -n "$(ls -1 | grep -E 'efi')" ]]; then
			EFI_ZHONGZHUAN="$(ls -1 |grep -Eo ".*combined-efi.*img.gz" |grep -v ".vm\|.vb\|.vh\|.qco\|root\|factory\|kernel")"
  		...
  		if [[ -n "$(ls -1 | grep -E 'squashfs')" ]]; then
			UP_ZHONGZHUAN="$(ls -1 |grep -Eo ".*combined.*img.gz" |grep -v ".vm\|.vb\|.vh\|.qco\|efi\|root\|factory\|kernel")"
