...
for x in "${t[@]}"; do
    find "${HOME_PATH}/feeds" "${HOME_PATH}/package" \
        ...
        -path "${HOME_PATH}/feeds/momo" -prune -o \
...
#删除dstheme中多余的主题和插件
rm -rf ${HOME_PATH}/feeds/dstheme/{luci-app-mosdns,luci-app-smartdns,openwrt-smartdns}
rm -rf ${HOME_PATH}/feeds/dstheme/relevance/openwrt-smartdns
echo "删除dstheme中多余的插件..."
#删除danshui中多余的主题和插件
rm -rf ${HOME_PATH}/feeds/danshui/{filebrowser,luci-app-mentohust,luci-app-nginx-pingos,luci-app-webd,qmodem,rblibtorrent,luci-app-passwall,luci-app-passwall2,luci-app-amlogic,luci-app-ssr-plus,luci-app-unblockneteasemusic,luci-app-homeproxy,luci-app-linkease,luci-app-istorex,luci-app-store,luci-app-istoreenhance,luci-app-quickstart,luci-app-nikki}
...
#  gitsvn https://github.com/281677160/common/blob/main/Share/luci-app-nginx-pingos/Makefile ${HOME_PATH}/feeds/dstheme/luci-app-nginx-pingos/Makefile
#fi
if [[ "${REPO_BRANCH}" == *"23.05"* ]]; then
  gitsvn https://github.com/coolsnowwolf/packages/tree/152022403f0ab2a85063ae1cd9687bd5240fe9b7/net/dnsproxy ${HOME_PATH}/feeds/packages/net/dnsproxy
  gitsvn https://github.com/coolsnowwolf/lede/tree/326599e3d08d7fe1dc084e1c87581cdf5a8e41a6/package/libs/libjson-c ${HOME_PATH}/package/libs/libjson-c
fi
if [[ "${REPO_BRANCH}" =~ (main|master|openwrt-25.12) ]]; then
  gitsvn https://github.com/openwrt/packages/tree/openwrt-25.12/lang/rust ${HOME_PATH}/feeds/packages/lang/rust
fi
}
...
if [[ `grep -c "CONFIG_PACKAGE_luci-theme-argon=y" ${HOME_PATH}/.config` -eq '1' ]]; then
  ...
  curl -fsSL https://raw.githubusercontent.com/zhtcl/openwrt-package/usb/argon/jpg/10.jpg -o ${HOME_PATH}/files/www/luci-static/argon/background/argon.jpg
  if [[ $? -ne 0 ]]; then
    echo "拉取文件错误,请检测网络"
    exit 1
  fi
